...
[![CRAN
status](https://www.r-pkg.org/badges/version/comparpq)](https://CRAN.R-project.org/package=comparpq)
<!-- badges: end -->
...
- **`tc_metrics_mock()`**: Compute accuracy metrics comparing taxonomic
  assignations against mock communities
- **`tc_metrics_mock_vec()`**: Vectorized version for efficient metric
  computation
- **`tc_points_matrix()`**: Create matrices for taxonomic comparison
  visualization

### Visualization Tools

- **`bubbles_pq()`**: Interactive bubble plots using Observable HQ for
  phyloseq objects
- **`tc_bar()` & `tc_circle()`**: Bar charts and circular plots for
  taxonomic comparisons
- **`rainplot_taxo_na()`**: Rain plots for visualizing NA patterns in
  taxonomic data

### Data Manipulation

- **Fake taxa creation**: `add_external_seq_pq()`,
  `add_shuffle_seq_pq()` for mock community preparation
- **Taxonomic table utilities**: `rename_ranks_pq()`,
  `select_ranks_pq()`, `resolve_taxo_conflict()`
- **Pattern replacement**: `taxtab_replace_pattern_by_NA()` for data
  cleaning
