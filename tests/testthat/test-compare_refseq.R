test_that("compare_refseq works with identical objects", {
  res <- compare_refseq(data_fungi_mini, data_fungi_mini, verbose = FALSE)
...
test_that("compare_refseq detects unique taxa and computes distances", {
  taxa_keep <- taxa_names(data_fungi_mini)[1:20]
  sub <- prune_taxa(taxa_keep, data_fungi_mini)
  res <- compare_refseq(data_fungi_mini, sub, verbose = FALSE)
...
test_that("compare_refseq errors without refseq slot", {
  pq_no_refseq <- data_fungi_mini
  pq_no_refseq@refseq <- NULL

  expect_error(
    compare_refseq(pq_no_refseq, data_fungi_mini, verbose = FALSE),
    "has no refseq slot"
  )
  expect_error(
    compare_refseq(data_fungi_mini, pq_no_refseq, verbose = FALSE),
    "has no refseq slot"
  )
})
...
test_that("compare_refseq warns with >2 list_phyloseq objects", {
  ...
  expect_message(
    compare_refseq(lpq, verbose = FALSE),
    "Using the first two"
  )
})

test_that("compare_refseq errors with <2 list_phyloseq objects", {
...
test_that("compare_refseq errors when physeq2 missing", {
  expect_error(
    compare_refseq(data_fungi_mini, verbose = FALSE),
    "physeq2.*required"
  )
})
