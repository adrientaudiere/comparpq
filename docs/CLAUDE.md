...
**comparpq** is an R package extending MiscMetabar and phyloseq for
comparing phyloseq objects in microbiome analysis. It focuses on
taxonomic comparison, accuracy metrics computation, and interactive
visualizations.

## Common Commands

You may propose to use the following commands when working with the
package. Please do not run them automatically; only suggest them when
appropriate.
...
- **phyloseq**: S4 class system for microbiome data (slots: @tax_table,
  @otu_table, @refseq, @sam_data)
...
- Parameter docs start with type:
  `@param x (Int, required) An integer vector of length 1`
- Add new topics to `_pkgdown.yml` reference section
- Run
  [`pkgdown::check_pkgdown()`](https://pkgdown.r-lib.org/reference/check_pkgdown.html)
  to verify all topics are indexed
