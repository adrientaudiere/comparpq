...
* `gg_bubbles_pq()` now accepts a `list_phyloseq` object as `physeq`, producing one bubble chart per phyloseq object combined via patchwork with a shared fill legend. New `diff_contour` parameter (requires ≥ 3 phyloseq objects) appends one pairwise comparison panel per pair of objects, highlighting taxa unique to each member of the pair with a red border contour. Color of unique taxa and border width are controlled by `diff_unique_color` and `diff_border_width`.
